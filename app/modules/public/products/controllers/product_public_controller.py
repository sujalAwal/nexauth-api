...
async def get_sale_products(db: AsyncSession = Depends(get_db)):
    """Get products on sale (public endpoint)."""
    try:
        service = ProductPublicService(db)
        products = await service.get_on_sale_products()
...
async def get_products_by_brand(brand_id: UUID, db: AsyncSession = Depends(get_db)):
...
async def get_product_by_id(product_id: UUID, db: AsyncSession = Depends(get_db)):
    ...
    try:
        ...
        await service.increment_visit_count(product_id)
